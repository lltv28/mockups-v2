# Internal AI Assessment Proof Slide

## Scope

Add one new slide immediately after the current slide 3, `Expert AIs`, and before `Why our clients love the Kodara model`. Build the slide in the existing Kodara design system. Keep `slides/deck.html` and its versioned preview copy, `slides/deck-july-2026.html`, synchronized.

## Message and copy

Use the final approved comparison copy:

- Eyebrow: `WE RAN THIS ON OURSELVES`
- Headline: `One extra step changes everything.`
- First path: `Ad` to `VSL` to `Booked call` to `35% Shows up` to `20% Closes`
- Second path: `Ad` to `AI assessment` to `Booked call` to `60% Shows up` to `33% Closes`
- Remove the visible `Model A` and `Model B` row labels. Keep the unchanged supporting statement that refers to Model A and Model B.
...
- Use the deck's white background, subtle green glow, Instrument Sans body type, and Instrument Serif headline and large proof figure.
...
- Fit the full composition within the 1600 by 900 desktop slide canvas without clipping.
- Keep both five-card paths aligned and easy to compare at a glance.
- At the existing mobile breakpoint, stack each path vertically and rotate or replace connectors so the reading direction remains clear.
- Preserve the deck's existing responsive scrolling and navigation behavior.

## Accessibility

- Use labeled sections and ordered lists for the two paths without adding visible row headings.
- Mark connector arrows and decorative icons as hidden from assistive technology.
- Maintain readable contrast and do not rely on color alone to distinguish the models.

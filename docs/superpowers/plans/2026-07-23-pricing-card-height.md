...
**Goal:** Remove the redundant 3-Pay total and make all three payment cards about 30% shorter with centered content.
...
- Modify `slides/tests/pitch-deck-tweaks.test.mjs`: assert the removed total and shared compact centered-card rule.
...
Expected: FAIL because the repeated total still exists and the cards do not yet use the compact centered layout.
...
- [ ] **Step 3: Tighten the price spacing**

Change each payment amount’s `margin-top` from `14px` to `8px` so all content fits cleanly inside the shorter cards.
...
- All three cards render about 158px tall on desktop and use the 105px compact height on mobile.
